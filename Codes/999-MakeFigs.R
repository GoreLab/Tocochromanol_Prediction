...
# mkdir
dir.save <- "RESULT/999-Tables_and_Figures_Final"
dir.create(dir.save, recursive = T)

# load phenotype data
f.ames <- "DATA_UPLOAD/AmesPheno.csv" # Ames Phenotype data
f.gb <- "DATA_UPLOAD/GbPheno.csv" # Gb Phenotype data
...
# load overlap
df.overlap.info <- fread("Summary_of_1702_lines.csv", data.table = F)
...
p <- p + scale_fill_manual(values = cols)
p <- p + theme(legend.key.height= unit(1, 'cm'))
ggplot2::ggsave(filename = paste0(dir.save, "/Fig2b_small.eps"), 
                plot = p, 
                device = cairo_ps, 
                dpi = 300, 
                width = 12 * 0.7,
...
list.cand <- as.data.frame(read_xlsx("tocochromanol_all_candidate_genes_combined_DW_20190624_with_two_genes.xlsx"))
...
df.gene.13 <- as.data.frame(read_xlsx("CandidateGeneNames.xlsx"))
