...
name.dir <- paste0("RESULT/3.2-GenPreCv_SinglePop_trans_GBLUP")
...
# load GRM
if ( pop == "ames" ) { GRM.df <- fread("RESULT/1.5-MakeGenReMat/GenReMat_ames.csv", data.table = F) }
if ( pop == "gb" ) { GRM.df <- fread("RESULT/1.5-MakeGenReMat/GenReMat_gb.csv", data.table = F) }
...
write.csv(df.save, file = paste0("RESULT/3.2-GenPreCv_SinglePop_trans_BLUP/yPred_Trait=", trait, "_Train=", pop, "_Model=", model, ".csv"))
