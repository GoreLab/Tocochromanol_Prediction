...
name.dir <- paste0("RESULT/2.3-GenPreFit_SinglePop_trans_GBLUP/Trait=", trait, "_Train=", pop, "_Model=", model)
dir.create(name.dir, recursive = TRUE)
name.dir.logfile <- paste0(name.dir, "/Logfile")
dir.create(name.dir.logfile)

# load GRM
GRM.df <- fread("RESULT/1.5-MakeGenReMat/GenReMat_all.csv", data.table = F)
...
# save
filename.save <- paste0("RESULT/2.3-GenPreFit_SinglePop_trans_GBLUP/yPred_trait=", trait, "_train=", pop, "_model=", model, ".csv")
