...
# setup
df.comb <- data.frame("Feature" = c("MAF", "MAF",
...
for ( i in 1:nrow(df.comb) ) {
   ...
   for ( chr in chr.all ) {
      ...
      ResList[[chr]] <- myFun.calc.GRM.info(X.012 = GenoMat[, tf], batch = 5000)
      gc(); gc()
   }
   ...
   # write the GRM
   f.save <- paste0(dir.save, "/GenReMat_use_12M_SNP_", feat, "_", grp, ".csv")
