...
name.dir <- paste0("RESULT/3.3-GenPreCv_SinglePop_trans_GBLUP_12M")
...
# load GRM
if ( pop == "ames" ) { GRM.df <- fread("RESULT/1.8-GRM_12M_EachPop/GenReMat_use_12M_SNP_Ames.csv", data.table = F) }
if ( pop == "gb" ) { GRM.df <- fread("RESULT/1.8-GRM_12M_EachPop/GenReMat_use_12M_SNP_Gb.csv", data.table = F) }
...
write.csv(df.save, file = paste0("RESULT/3.3-GenPreCv_SinglePop_trans_GBLUP_12M/yPred_Trait=", trait, "_Train=", pop, "_Model=", model, ".csv"))
